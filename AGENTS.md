...
- `src/components/` — component implementations. Current set: `Accolades`, `Accordion`, `AlertDialog`, `Anchor`, `Avatar`, `AvatarBadge`, `Badge`, `Button`, `ButtonGroup`, `Checkbox`, `CheckboxGroup`, `Collapsible`, `Container`, `Dialog`, `FlexContainer`, `Footer`, `Form`, `GridContainer`, `Header`, `Hero`, `Label`, `Navigation`, `Pagination`, `PasswordInput`, `Progress`, `RadioGroup`, `Recents`, `Secure`, `Select`, `Separator`, `Slider`, `Switch`, `TextInput`, `Textarea`, `Tooltip`, `Typography`.
...
- `Form.tsx` is the one exception to "single cohesive component per Radix primitive": it exports `Form`, `FormField`, `FormLabel`, `FormControl`, `FormMessage` as separate thin wrappers (each just applies a class name to the matching Radix `Form.*` piece) rather than one all-in-one component, because a form's shape is inherently open-ended (an arbitrary number of fields with arbitrary controls) unlike Dialog's fixed trigger/title/description/footer slots. Compose forms by hand from these plus a plain `<input>`/`<select>`/etc. or another of this library's components via `FormControl asChild`, and this library's own `Button type="submit"` (`Form.Submit` isn't wrapped — it doesn't support `asChild`, and any `<button type="submit">` inside a `<form>` already submits it, so there's nothing it adds). Deliberately has no field-schema system and no bundled validation library — just Radix's native browser constraint validation (`required`, `type`, `pattern`, etc.) surfaced through `FormMessage`'s built-in `match` matchers (`valueMissing`, `typeMismatch`, `tooShort`, etc.).
...
- Watch for prop names that collide with a native HTML attribute of a _different_ type than what you want. `title` (native: plain string, tooltip) is the one that has bitten us so far — `Hero`'s `title?: ReactNode` needed `Omit<HTMLAttributes<HTMLElement>, "title">` on its props interface to compile.
...
- `title` is `Components/<ComponentName>` for most components — a flat group, no further sub-categorization. Two exceptions get their own top-level group instead: `Dialog`/`AlertDialog` use `Dialogs/<Name>`, and larger, section-level compositions use `Blocks/<Name>` (`Accolades`, `Hero`). Plain utility functions (`src/lib/*.ts`, not components) use `Utilities/<functionName>` and go in `src/stories/`, same as component stories, with a small local demo component rendering the input/output rather than the utility itself (see `Cn.stories.tsx`). `preview.tsx`'s `storySort` orders the groups Docs, Components, Blocks, Containers, Dialogs, Utilities, alphabetical within each.
