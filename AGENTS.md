...
- `src/components/` — component implementations. Current set: `Accolades`, `Accordion`, `AlertDialog`, `Anchor`, `Avatar`, `Badge`, `BlogPost`, `Button`, `ButtonGroup`, `Checkbox`, `CheckboxGroup`, `Collapsible`, `Container`, `Dialog`, `FlexContainer`, `Footer`, `Form`, `GridContainer`, `Header`, `Hero`, `Label`, `ListCard`, `Navigation`, `Pagination`, `PasswordInput`, `PostCard`, `ProfileCard`, `ProfilePage`, `Progress`, `RadioGroup`, `Recents`, `Secure`, `Select`, `Separator`, `Slider`, `Switch`, `TextInput`, `Textarea`, `Tooltip`, `Typography`.
...
- `Form` optionally integrates `react-hook-form`, but unlike `react-paginate` it's never imported at runtime by this library at all (only as a type: `import type { UseFormReturn } from "react-hook-form"`) and there's no lazy-loading machinery for it. Reason: `react-paginate`'s lazy-`import()` pattern works because it's a plain component you can conditionally render behind a `Suspense` boundary once the import resolves; `useForm()` is a _hook_, and hooks can't be called conditionally or after an async import settles (rules of hooks). So the only correct way to make a hook-based dependency truly optional is for the consumer to call the hook themselves (in their own app code, where the package is actually installed) and hand the result to the component via a prop — `Form`'s `form?: UseFormReturn<Record<string, unknown>>`. When present, fields use `form.register(name, rules)` and errors read from `form.formState.errors`; when absent, fields are plain uncontrolled native inputs validated via the browser's own Constraint Validation API. `react-hook-form` is a devDependency (for our own typecheck) and an optional peer dependency (`peerDependenciesMeta`), matching `react-paginate`'s existing entries but without react-paginate's runtime lazy-import wrapper - there's nothing to lazy-load. If a `form` is given, `Form` also sets `noValidate` on the root `<form>`: native `required`/`type="email"` etc. block the `submit` event from firing at all when invalid, which would also block `form.handleSubmit` (and therefore react-hook-form's own validation) from ever running - `noValidate` hands validation entirely to react-hook-form instead of double-validating through two independent systems.
- `Form` renders its own native `<input>`/`<textarea>`/`<select>`/`<input type="radio"|"checkbox">` elements rather than reusing `TextInput`/`Checkbox`/`RadioGroup`/`Select`. Reason: `@radix-ui/react-form`'s `Form.Control` tracks validity by reading the browser's native `ValidityState` API directly off the wrapped DOM node (confirmed by reading `@radix-ui/react-form`'s source) - it only works when the wrapped element actually is a native form-associated control. `Checkbox`/`RadioGroup`/`Select` are Radix-primitive-based (buttons/listboxes), not native controls, so they can't participate in Radix Form's automatic `Form.Message match="valueMissing"`-style validation, and native elements are also what `react-hook-form`'s `register()` is designed to spread onto. Visual consistency with the rest of the library (same border/background/focus-ring tokens) is maintained by hand in `form.css` rather than by sharing `checkbox.css`/`radio-group.css`/`text-input.css` directly.
...
- **Dark mode**: `theme.css` registers `@custom-variant dark (&:where([data-theme="dark"], [data-theme="dark"] *));`, which makes Tailwind's `dark:` prefix mean "under `data-theme="dark"`" instead of its default `prefers-color-scheme` media query. Every component applies its own `dark:` classes explicitly, one per `default-*` usage (see any file in `src/styles/components/`) — there's no automatic inversion, so a component or property can deviate from the standard pairing if it needs to. The standard pairing (mirror across the midpoint) is: 50↔950, 100↔900, 200↔800, 300↔700, 400↔600, and 500 is left alone (skip the `dark:` class entirely — it already reads correctly in both themes). When adding a new component, apply that pairing to every `default-*` utility unless there's a specific reason to deviate. It's opt-in via the attribute only, deliberately not tied to `prefers-color-scheme` — see the README's "Dark mode" section for why. Storybook previews it via `.storybook/preview.tsx`'s `withThemeByDataAttribute` decorator (from `@storybook/addon-themes`, toggling the same attribute, defaulting to dark) and `.storybook/preview.css` (a plain `:root[data-theme="dark"] body` rule setting the preview canvas's own background, mirroring the same 50↔950 pairing — it's plain CSS, not run through Tailwind, since it's docs-site-only and isn't shipped in `main.css`).
